...
const messages: Record<AppLocale, Record<string, string>> = {
  'zh-CN': {
    ...
    'lobby.create.domination': '创建抢点棋房间',
    ...
    'lobby.go.board_size': '围棋棋盘大小',
    'lobby.create.xiangqi': '创建象棋房间',
    'lobby.create.texas_holdem': '创建德州扑克房间',
    'lobby.training': '训练模式',
    'training.title': '训练模式',
    'training.sandbox_note': '训练沙盒：使用确定性本地随机种子，不走实时 commit/reveal。',
    'training.phase1.title': '阶段 1 - 公开棋盘游戏',
    'training.phase2.title': '阶段 2 - 训练沙盒',
    'training.mode.training': '训练',
    'training.mode.sandbox': '沙盒',
    'training.summary': '{mode}（{game}）- 状态：{status}，下一手：{next}',
    'training.reset': '重置{game}{scope}',
    'training.go.board_size': '棋盘大小',
    'training.go.size_9': '9 路',
    'training.go.size_13': '13 路',
    'training.go.size_19': '19 路',
    'training.label.worker': '工人',
    'training.label.card': '卡牌',
    'training.label.target': '目标',
    'training.label.guess': '猜测',
    'training.label.clue': '提示',
    'training.label.count': '数量',
    'training.label.phase': '阶段',
    'training.label.cluer': '出题方',
    'training.label.guesser': '猜词方',
    'training.label.quantity': '数量',
    'training.label.side': '侧牌',
    'training.santorini.prompt.build': '请选择建造格。',
    'training.santorini.prompt.destination': '请选择移动目标格。',
    'training.santorini.prompt.select_worker': '请选择你的工人。',
    'training.santorini.prompt.setup': '部署阶段：先选择工人，再点击落位格。',
    'training.onitama.cards':
      '{blackLabel}方卡牌：{blackCards} | {whiteLabel}方卡牌：{whiteCards} | {sideLabel}：{side}',
    'training.onitama.selected': '已选择 {x},{y}',
    'training.onitama.select_piece': '请选择你的棋子',
    'training.battleship.status':
      '{phaseLabel}：{phase}，{blackLabel}方击沉：{sunkBlack}，{whiteLabel}方击沉：{sunkWhite}',
    'training.battleship.phase.placement': '部署',
    'training.battleship.phase.playing': '对战',
    'training.battleship.outgoing_shots': '你的射击面板',
    'training.battleship.fleet_view': '你的舰队视图',
    'training.yahtzee.overview':
      '已掷骰：{rollsUsed} | 总分 {blackLabel}/{whiteLabel}：{totalBlack}/{totalWhite}',
    'training.codenames.phase_status':
      '{phaseLabel}：{phase}，{cluerLabel}：{cluer}，{guesserLabel}：{guesser}',
    'training.codenames.active_clue': '当前提示：{word}（{count}）',
    'training.codenames.phase.clue': '提示阶段',
    'training.codenames.phase.guess': '猜词阶段',
    'training.codenames.mark.agent': '特工',
    'training.codenames.mark.assassin': '刺客',
    'training.codenames.mark.neutral': '中立',
    'training.cards.eight_suit': '8 的花色',
    'training.liars.current_bid': '当前出价：',
    'training.liars.none': '无',
    'training.backgammon.dice': '骰子：{dice} | 剩余：{remaining}',
    'training.backgammon.na': '无',
    'training.backgammon.none': '无',
    'training.backgammon.bar_off':
      '暂存区 白/黑：{barWhite}/{barBlack} | 出局区 白/黑：{offWhite}/{offBlack}',
    'training.backgammon.legal_moves': '合法走法（{count}）：',
    'training.backgammon.no_legal_moves': '当前无合法走法（阻塞回合会自动停一手）。',
    ...
    'room.go.board_size': '棋盘大小: {size} 路',
    ...
    'room.battleship.place_fleet': '提交舰队布局',
    ...
    'room.yahtzee.upper_totals': '上区小计 • 黑: {black} • 白: {white}',
    'room.yahtzee.upper_bonus': '上区奖励(+35) • 黑: {black} • 白: {white}',
    'room.yahtzee.category': '类别',
    'room.domination.scores': '得分 • 黑: {black} • 白: {white}',
    'room.domination.pieces': '棋子数 • 黑: {black} • 白: {white}',
    'room.domination.controls': '控制点 • 黑: {black} • 白: {white}',
    'room.love_letter.waiting_rng': '等待双方完成随机承诺与揭示后洗牌发牌',
    'room.love_letter.play': '出牌',
    'room.love_letter.round': '回合: {current} • 目标令牌: {target}',
    'room.love_letter.tokens': '令牌 • 黑: {black} • 白: {white}',
    'room.love_letter.draw_pile': '牌堆剩余: {count}',
    'room.love_letter.removed_face_up': '公开移除牌: {cards}',
  ...
  'en-US': {
    ...
    'lobby.create.domination': 'Create Domination Room',
    ...
    'lobby.go.board_size': 'Go board size',
    ...
    'training.phase1.title': 'Phase 1 - Public Board Games',
    'training.phase2.title': 'Phase 2 - Training Sandbox',
    'training.mode.training': 'Training',
    'training.mode.sandbox': 'Sandbox',
    'training.summary': '{mode} ({game}) - status: {status}, next: {next}',
    'training.reset': 'Reset {game} {scope}',
    'training.go.board_size': 'Board size',
    'training.go.size_9': '9x9',
    'training.go.size_13': '13x13',
    'training.go.size_19': '19x19',
    'training.label.worker': 'Worker',
    'training.label.card': 'Card',
    'training.label.target': 'Target',
    'training.label.guess': 'Guess',
    'training.label.clue': 'Clue',
    'training.label.count': 'Count',
    'training.label.phase': 'Phase',
    'training.label.cluer': 'Cluer',
    'training.label.guesser': 'Guesser',
    'training.label.quantity': 'Quantity',
    'training.label.side': 'Side',
    'training.santorini.prompt.build': 'Click a build cell.',
    'training.santorini.prompt.destination': 'Click destination cell.',
    'training.santorini.prompt.select_worker': 'Click one of your workers.',
    'training.santorini.prompt.setup': 'Setup: select worker + click a placement cell.',
    'training.onitama.cards':
      '{blackLabel} cards: {blackCards} | {whiteLabel} cards: {whiteCards} | {sideLabel}: {side}',
    'training.onitama.selected': 'Selected {x},{y}',
    'training.onitama.select_piece': 'Select one of your pieces',
    'training.battleship.status':
      '{phaseLabel}: {phase}, sunk by {blackLabel}: {sunkBlack}, sunk by {whiteLabel}: {sunkWhite}',
    'training.battleship.phase.placement': 'placement',
    'training.battleship.phase.playing': 'playing',
    'training.battleship.outgoing_shots': 'Your outgoing shots',
    'training.battleship.fleet_view': 'Your fleet view',
    'training.yahtzee.overview':
      'Rolls used: {rollsUsed} | totals {blackLabel}/{whiteLabel}: {totalBlack}/{totalWhite}',
    'training.codenames.phase_status':
      '{phaseLabel}: {phase}, {cluerLabel}: {cluer}, {guesserLabel}: {guesser}',
    'training.codenames.active_clue': 'Active clue: {word} ({count})',
    'training.codenames.phase.clue': 'clue',
    'training.codenames.phase.guess': 'guess',
    'training.codenames.mark.agent': 'AGENT',
    'training.codenames.mark.assassin': 'ASSASSIN',
    'training.codenames.mark.neutral': 'NEUTRAL',
    'training.cards.eight_suit': '8 suit',
    'training.liars.current_bid': 'Current bid:',
    'training.liars.none': 'none',
    'training.backgammon.dice': 'Dice: {dice} | Remaining: {remaining}',
    'training.backgammon.na': 'n/a',
    'training.backgammon.none': 'none',
    'training.backgammon.bar_off': 'Bar W/B: {barWhite}/{barBlack} | Off W/B: {offWhite}/{offBlack}',
    'training.backgammon.legal_moves': 'Legal moves ({count}):',
    'training.backgammon.no_legal_moves': 'No legal move right now (auto-pass handles blocked turns).',
    ...
    'room.go.board_size': 'Board size: {size}x{size}',
    ...
    'room.battleship.place_fleet': 'Submit Fleet',
    ...
    'room.yahtzee.upper_totals': 'Upper subtotal • Black: {black} • White: {white}',
    'room.yahtzee.upper_bonus': 'Upper bonus (+35) • Black: {black} • White: {white}',
    'room.yahtzee.category': 'Category',
    'room.domination.scores': 'Score • Black: {black} • White: {white}',
    'room.domination.pieces': 'Pieces • Black: {black} • White: {white}',
    'room.domination.controls': 'Control • Black: {black} • White: {white}',
    ...
    'room.love_letter.round': 'Round: {current} • Token target: {target}',
    'room.love_letter.tokens': 'Tokens • Black: {black} • White: {white}',
    'room.love_letter.draw_pile': 'Deck left: {count}',
    'room.love_letter.removed_face_up': 'Face-up removed cards: {cards}',
    ...
    'enum.game.domination': 'domination',
